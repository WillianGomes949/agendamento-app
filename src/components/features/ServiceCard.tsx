...
export function ServiceCard({ servico, onEdit, onDelete, className }: ServiceCardProps) {
  ...
  // ✅ Usa funções de formatação para garantir exibição correta
  ...
  // ✅ Formata data e horário corretamente
  ...
  return (
    ...
    >
      <div className="relative flex-1">
        {/* Conteúdo clicável — Link isolado dos botões */}
        <Link 
        ...
        >
          <div className="flex justify-between items-start">
            <div className="min-w-0">
              <h3 className="font-semibold text-gray-900 line-clamp-1">{clienteNome}</h3>
              <p className="text-xs text-gray-500 mt-0.5">OS: {ordemServico}</p>
            </div>
          ...
          <div className="space-y-2.5 text-sm text-gray-600">
            <div className="flex items-center gap-2.5">
              ...
              <span>{dataFormatada}{diaSemana ? ` • ${diaSemana}` : ""}</span>
            </div>
            <div className="flex items-center gap-2.5">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-mono uppercase">{veiculoPlaca} {veiculoModelo && <span className="text-gray-400">({veiculoModelo})</span>}</span>
            </div>
            <div className="flex items-center gap-2.5">
              <MapPin className="w-4 h-4 text-gray-400 shrink-0" aria-hidden="true" />
              <span className="line-clamp-1">{enderecoCidade}{enderecoEstado ? `/${enderecoEstado.toUpperCase()}` : ""}</span>
            </div>
          </div>
        ...
        {/* Botões de Ação — FORA do Link, posicionados absolute */}
        {hasActions && (
          <div 
